 ...
 *
 * <p>规则顺序保持不变。相同维度集合和滚动窗口只能定义一次，空规则集合表示不限制签发频率。
 *
...
public record IssueRateLimitPolicy(List<IssueRateLimitRule> rules) {
     ...
     * @throws IllegalArgumentException 当存在相同维度集合和窗口的重复规则时
     */
    public IssueRateLimitPolicy {
        ...
        for (IssueRateLimitRule rule : rules) {
            RuleIdentity identity = new RuleIdentity(rule.dimensions(), rule.window());
            if (!identities.add(identity)) {
                throw new IllegalArgumentException("duplicate issue rate limit rule for dimensions " + rule.dimensions()
                        + " and window " + rule.window());
            }
        }
    }
    ...
    private record RuleIdentity(Set<IssueLimitDimension> dimensions, Duration window) {}
}
