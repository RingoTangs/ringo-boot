 ...
 * 不可变 {@link IssueRateLimitConstraint} 集合一次性提交给 {@link IssueRateLimitBackend}。
 *
 * <p>没有规则匹配时直接返回 {@link IssueLimitResult.Allowed}，不会访问后端。该类本身不可变；整体线程安全性还依赖规则实现和后端
 * 满足各自的线程安全契约。
 */
public final class IssueRateLimitManager implements IssueRateLimiter {
     ...
     * @param backend 原子执行已解析约束的后端
     * @throws NullPointerException 当规则集合、任一规则、规则定义字段或后端为 {@code null} 时
