...
public final class IssueLimitManager implements IssueLimiter {
     ...
     * 启动时复制并校验完成的不可变规则快照。
     */
    private final List<IssueLimitRule> rules;
    ...
    public IssueLimitManager(List<IssueLimitRule> rules, IssueLimitStore store) {
        ...
        if (this.rules.isEmpty()) {
            throw new MissingIssueLimitRuleException();
        }
        Set<String> ids = new HashSet<>();
        for (IssueLimitRule rule : this.rules) {
            Objects.requireNonNull(rule, "rule must not be null");
            IssueLimitValidator.validateRuleDefinition(rule.id(), rule.maxIssues(), rule.window());
            if (!ids.add(rule.id())) {
     ...
     * @throws IssueLimitStoreException       当底层限流状态操作失败时
     */
    @Override
    public IssueLimitResult acquire(IssueContext context, Instant requestedAt) throws IssueLimitStoreException {
