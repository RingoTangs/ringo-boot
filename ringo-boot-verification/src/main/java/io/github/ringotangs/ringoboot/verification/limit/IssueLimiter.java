 ...
 * 验证码服务使用的签发频率限制入口。
 *
 * <p>默认实现由 {@link RuleBasedIssueLimiter} 提供。签发服务负责创建上下文，限流器只负责检查和消费额度。
 ...
 * {@link IssueLimitStoreException}。正常限流始终通过 {@link IssueLimitResult.Throttled} 返回。
 *
 * <p>默认管理器采用严格拒绝策略，没有规则覆盖当前验证码键时抛出 {@link MissingIssueLimitRuleException}。不需要限制签发频率时
 * 可以使用 {@link #permitAll()}；Spring Boot 自动配置在应用没有提供规则或限流器时使用该实现。
 */
@FunctionalInterface
public interface IssueLimiter {
     ...
     * @throws MissingIssueLimitRuleException 当没有规则覆盖当前验证码键时
     * @throws IssueLimitStoreException       当底层限流操作失败时
     */
    IssueLimitResult acquire(IssueContext context, Instant requestedAt) throws IssueLimitException;
}
