 ...
 * 同一渠道和验证码业务键的写入、比对、尝试次数扣减和消费必须具备原子性。
 *
 *
...
public interface VerificationStore {
     ...
     * @throws VerificationStoreException 当底层存储操作失败时
     */
    VerifyResult verifyAndConsume(VerificationStoreKey key, String code, Instant verifiedAt)
            throws VerificationStoreException;
     ...
     * @throws VerificationStoreException 当底层存储操作失败时
     */
    boolean invalidate(VerificationStoreKey key, String code) throws VerificationStoreException;
}
