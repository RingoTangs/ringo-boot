 ...
 * 表示一次验证码签发流程共享的上下文。
 *
 * <p>验证码键和渠道描述当前签发流程；扩展属性用于携带 IP、设备、账号、租户等由应用定义的信息。
 ...
 */
public record IssueContext(VerificationKey key, VerificationChannel channel, Map<String, String> attributes) {

    /**
     * 创建签发上下文并防御性复制扩展属性。
     *
     * @throws NullPointerException 当验证码键、渠道、属性集合、属性名或属性值为 {@code null} 时
     ...
     * @return 不含扩展属性的签发上下文
     */
    public static IssueContext of(VerificationKey key, VerificationChannel channel) {
        return new IssueContext(key, channel, Map.of());
    }
    ...
    public IssueContext with(String name, String value) {
        ...
        copy.put(name, value);
        return new IssueContext(key, channel, copy);
    }
